...
class YoloLayer(nn.Module):
    ...
    def forward(self, input, image_size):
        if self.training:
    ...
    def reset_input_size(self, input, image_size):
        dev = input.device
        ...
        scale_w = scale_w.reshape([1, 1, num_ancs, 1, 1]) / image_size[0]
        scale_h = scale_h.reshape([1, 1, num_ancs, 1, 1]) / image_size[1]
        self.scale_w = scale_w.to(dev)
        self.scale_h = scale_h.to(dev)

        off_c = torch.arange(self.anc_cols, dtype=torch.float32)
        off_r = torch.arange(self.anc_rows, dtype=torch.float32)
        self.anc_off_c = off_c.reshape([1, 1, 1, 1, self.anc_cols]).to(dev)
        self.anc_off_r = off_r.reshape([1, 1, 1, self.anc_rows, 1]).to(dev)
...
class Yolov3Model(nn.Module):
    def __init__(self, num_cls, image_size, anchors, masks):
        ...
        for i, input in enumerate(example_output):
            yolo_layers[i].reset_input_size(input, image_size)
        self.yolo_mods = nn.ModuleList(yolo_layers)

    def forward(self, input):
        outputs = self.backbone(input)
        if self.training:
            self.image_size = torch.tensor((input.shape[3], input.shape[2]))
        output0 = self.yolo_mods[0](outputs[0], self.image_size)
        output1 = self.yolo_mods[1](outputs[1], self.image_size)
        output2 = self.yolo_mods[2](outputs[2], self.image_size)
